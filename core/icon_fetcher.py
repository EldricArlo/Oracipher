...
class IconFetcher:
    ...
    def _get_raw_default_pixmap() -> QPixmap:
        ...
        pixmap = QPixmap(QSize(IconFetcher.TARGET_ICON_SIZE, IconFetcher.TARGET_ICON_SIZE))
    ...
    def get_default_icon_base64() -> str | None:
        ...
        try:
            ...
            logger.error(f"Failed to generate Base64 for default icon: {e}", exc_info=True)
            return None
        ...
        if base64_str:
            try:
                ...
                if not raw_pixmap.loadFromData(byte_array):
                    logger.warning("Failed to load pixmap from byte array, data might be corrupt. Falling back to default.")
                    raw_pixmap = IconFetcher._get_raw_default_pixmap()
            except Exception as e:
                logger.warning(f"Failed to create QPixmap from Base64: {e}. Falling back to default.")
                raw_pixmap = IconFetcher._get_raw_default_pixmap()
        else:
            raw_pixmap = IconFetcher._get_raw_default_pixmap()
