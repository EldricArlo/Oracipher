...
def check_and_migrate_schema(db_path: str) -> None:
    ...
    try:
        ...
        cursor_check.execute("SELECT sql FROM sqlite_master WHERE type='table' AND name='entries'")
        ...
        if result:
            ...
            if "UNIQUE" in create_sql.upper():
                ...
                logger.warning("Old database schema detected (with UNIQUE constraint on entries.name).")
                logger.warning(f"Backing up old database to: {backup_path}")
                os.rename(db_path, backup_path)
                logger.warning("Backup complete. A new, compatible database will be created.")
