...
class MainWindowController(QObject):
    ...
    def _connect_signals(self) -> None:
        ...
        self.sidebar_view.generate_password_button.clicked.connect(
            self._open_generator_dialog
        )
        self.sidebar_view.settings_button.clicked.connect(self._open_settings_dialog)
        self.sidebar_view.minimize_button.clicked.connect(
            self.main_app_window.showMinimized
        )
    ...
    def _filter_and_display_entries(self) -> None:
        ...
        if self.current_search_term:
            ...
            filtered_by_search = [
                entry
                for entry in filtered_by_cat
                if (
                    term in entry.get("name", "").lower()
                    or term in entry.get("category", "").lower()
                    or term in entry.get("details", {}).get("username", "").lower()
                    or term in entry.get("details", {}).get("url", "").lower()
                    or term in entry.get("details", {}).get("notes", "").lower()
                    or term in entry.get("details", {}).get("backup_codes", "").lower()
                )
            ]
        else:
    ...
    def on_delete_entry(self, entry_id: int) -> None:
        entry_to_delete = next(
            (e for e in self.all_entries if e["id"] == entry_id), None
        )
    ...
    def _open_add_dialog(self) -> None:
        ...
        if dialog.exec():
            ...
            if not data["name"]:
                CustomMessageBox.information(
                    self.main_app_window,
                    t.get("msg_title_input_error"),
                    t.get("msg_empty_name_error"),
                )
                return
    ...
    def _open_settings_dialog(self) -> None:
        ...
        if dialog.exec():
            ...
            if (
                ...
                or new_settings["auto_lock_timeout_minutes"] != auto_lock_minutes
            ):
                new_settings["auto_lock_enabled"] = auto_lock_enabled
    ...
    def _handle_change_password(self):
        ...
        if reason in error_map:
            CustomMessageBox.information(
                self.main_app_window,
                t.get("msg_title_pass_change_fail"),
                t.get(error_map[reason]),
            )
            return
        ...
        def on_success(success: bool):
            if success:
                CustomMessageBox.information(
                    self.main_app_window,
                    t.get("msg_title_pass_change_success"),
                    t.get("msg_pass_change_success"),
                )
            else:
                CustomMessageBox.information(
                    self.main_app_window,
                    t.get("msg_title_pass_change_fail"),
                    t.get("msg_pass_change_fail_old_wrong"),
                )
