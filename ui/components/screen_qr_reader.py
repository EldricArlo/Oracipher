...
class ScreenQRReader(QWidget):
    ...
    def __init__(self):
        ...
        if not primary_screen:
            logger.error(
                "Could not get primary screen! QR scan feature may be unavailable."
            )
        ...
        logger.info(
            f"Screen QR reader initialized. Device pixel ratio: {self.pixel_ratio}"
        )
    ...
    def capture_and_decode(self) -> None:
        ...
        try:
            ...
            logger.error(
                f"An error occurred during QR capture/decode: {e}", exc_info=True
            )
            self.scan_cancelled.emit()
