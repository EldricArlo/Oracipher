...
class GeneratorWindow(QDialog):
    ...
    def init_ui(self) -> None:
        ...
        title_label = QLabel(t.get('gen_title')); title_label.setObjectName("dialogTitle")
        
        display_layout = QHBoxLayout()
        self.password_display = QLineEdit(); self.password_display.setReadOnly(True)
        self.password_display.setObjectName("fieldValueDisplay"); self.password_display.setAlignment(Qt.AlignmentFlag.AlignCenter)
        copy_button = QPushButton(t.get('button_copy'))
        copy_button.clicked.connect(self.copy_to_clipboard)
        display_layout.addWidget(self.password_display); display_layout.addWidget(copy_button)
        
        options_layout = QGridLayout()
        self.length_slider = QSlider(Qt.Orientation.Horizontal)
        self.length_slider.setRange(8, 64); self.length_slider.setValue(16)
        self.length_slider.valueChanged.connect(self.update_options)
        self.length_label = QLabel()
        
        self.uppercase_check = QCheckBox(t.get('gen_check_upper')); self.uppercase_check.setChecked(True)
        self.numbers_check = QCheckBox(t.get('gen_check_num')); self.numbers_check.setChecked(True)
        self.symbols_check = QCheckBox(t.get('gen_check_sym')); self.symbols_check.setChecked(True)
        ...
        options_layout.addWidget(self.length_label, 0, 0); options_layout.addWidget(self.length_slider, 0, 1)
        ...
        done_button = QPushButton(t.get('button_done')); done_button.setObjectName("cancelButton")
        done_button.clicked.connect(self.accept)
        button_layout.addWidget(done_button)
        
        main_layout.addWidget(title_label); main_layout.addLayout(display_layout)
        main_layout.addLayout(options_layout); main_layout.addStretch(); main_layout.addLayout(button_layout)
