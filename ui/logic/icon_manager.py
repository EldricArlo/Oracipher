    ...
    def fetch_icon_from_url(self) -> None:
        ...
        if not url:
            CustomMessageBox.information(
                self.parent, t.get("msg_title_input_error"), t.get("error_url_required")
            )
            return
        ...
        if file_path:
            try:
                with open(file_path, "rb") as f:
                    self.current_icon_base64 = base64.b64encode(f.read()).decode(
                        "utf-8"
                    )
